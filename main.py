...
def confirm(msg: catbot.Message):
    ...
    user_info = user_info_query['query']['users']
    for user in user_info:
        if 'missing' in user.keys():
            bot.send_message(msg.chat.id, text=config['messages']['confirm_user_not_found'].format(
                name=mw_username))
            return
        if user['editcount'] >= 50 and \
                time.time() - timegm(time.strptime(user['registration'], '%Y-%m-%dT%H:%M:%SZ')) > 7 * 86400:
            break
    else:
